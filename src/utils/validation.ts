...
export const parseAndValidateArgs = (args: ArgumentABI[], customTypes: FieldABI[], txParams: any[]): any[] => {
    if( args.length != txParams.length ) throw new Error(`the arguments should be ${args.length}`)
    return args.map(({type, name}, index)=> {
        return parseAndValidateArg(name, type, txParams[index], customTypes);
    })
}
...
export const parseAndValidateArg = (name: string, type: ParamsType, value: any, customTypes: FieldABI[]):any => {
    ...
    switch (type){
        ...
        case "i32":
            parsedNumber = parseInt(value);
            if(isNaN(parsedNumber)) throw new Error(`param ${name} should be a integer - value ${value}`)
            if( parsedNumber > SIGNED_INT_MAX[type] || parsedNumber < (-1*SIGNED_INT_MAX[type]) ) throw new Error(`param ${name} should be a ${type} - value ${value}`)
            return parsedNumber;
    ...
    if(customTypeDef.length){
        ...
        switch (type){
            case "object":
                const obj = value as {[key:string]: any}
                return fields.map(({name: customTypeFieldName, type: customTypeFieldType }) => parseAndValidateArg(`${name}.${customTypeFieldName}`, customTypeFieldType, obj[customTypeFieldName], customTypes))
            default:
