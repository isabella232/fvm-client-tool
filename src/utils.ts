...
function attachConstructor(instance: any, name: string, args: ArgumentABI[], rtns: ReturnABI[], customTypes: FieldABI[]) {
  instance[name] = async (...fullArgs: any[]) => {
    if (instance.address) throw new Error("you cannot create a new instance because this object has already one attached");

    if (!instance.cid) throw new Error("cid is not present");

    const [from, value, txArgs = {}] = fullArgs;

    const params = cbor.encode(parseAndValidateArgs(args, customTypes, txArgs));
...
function attachMethod(instance: any, name: string, args: ArgumentABI[], rtns: ReturnABI[], customTypes: FieldABI[]) {
  instance[name] = async (...fullArgs: any[]) => {
    if (!instance.address)
      throw new Error("you need to create a new instance of the smart contract first or load the address of an existing one");
